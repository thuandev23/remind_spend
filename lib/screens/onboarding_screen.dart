...
class OnboardingScreen extends StatefulWidget {
  ...
  const OnboardingScreen({super.key, this.onComplete, this.pullService});
...
class _OnboardingScreenState extends State<OnboardingScreen>
  ...
  Future<void> _init() async {
    ...
    if (mfr.type == ManufacturerType.ios) {
      ...
      final isDone = prefs.getBool('ios_setup_complete') ?? false;
  ...
  Future<void> _onGrantNotification() async {
    ...
    if (_mfrInfo?.type == ManufacturerType.ios) {
      // IosShortcutStep handles requestLocalNotificationPermission directly.
      return;
    }
    await BridgeService.requestPostNotificationsPermission();
    await BridgeService.requestPermission();
    // Android: permissionStatusStream auto-detects when user grants access
  }
  ...
  Widget _buildStepIndicator() {
    ...
    if (isIos) {
      return Row(
        children: [OnboardingStepDot(label: '1', active: true, done: false)],
      );
    }
    return Row(
      children: [
